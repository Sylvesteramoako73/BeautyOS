      ...
      <div className="grid grid-cols-3 gap-5">
                        ...
                        <td className="text-right">
                          {inCart ? (
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => changeQty(svc.id, -1)} className="btn-ghost h-6 w-6 p-0 justify-center"><Minus className="h-3 w-3" /></button>
                              <span className="text-sm font-semibold w-5 text-center">{inCart.quantity}</span>
                              <button onClick={() => changeQty(svc.id, 1)}  className="btn-ghost h-6 w-6 p-0 justify-center"><Plus  className="h-3 w-3" /></button>
                            </div>
                          ) : (
                ...
                <div className="max-h-48 overflow-y-auto divide-y divide-gray-100">
                  {filteredClients.map(c => (
                    <div key={c.id} onClick={() => setClientId(c.id)} className={cn('flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-gray-50', clientId === c.id && 'bg-gray-900 text-white hover:bg-gray-900')}>
                      ...
                      {clientId === c.id && <Check className="h-4 w-4 text-white" />}
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="divide-y divide-gray-100">
                  {staff.map(m => (
                    <div key={m.id} onClick={() => setStaffId(m.id)} className={cn('flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-gray-50', staffId === m.id && 'bg-gray-900 text-white hover:bg-gray-900')}>
                      ...
                      {staffId === m.id && <Check className="h-4 w-4 text-white" />}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4">
                  ...
                  {PAYMENT_METHODS.map(pm => {
                    ...
                    return (
                      <div key={pm.id} onClick={() => setPayMethod(pm.id)} className={cn('flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-gray-50', payMethod === pm.id && 'bg-gray-900 hover:bg-gray-900')}>
                        ...
                        {payMethod === pm.id && <Check className="h-4 w-4 text-white" />}
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="card p-4">
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                  <Gift className="h-4 w-4 text-gray-400" /> Gift Card
                </p>
                {gcApplied ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      ...
                      <button onClick={applyGiftCard} disabled={gcLoading || !gcCode} className="btn-secondary px-4 disabled:opacity-40">
                        {gcLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Apply'}
                      </button>
                    </div>
        ...
        <div className="card overflow-hidden h-fit sticky top-6">
              ...
              <div className="space-y-2 mb-4">
                {cart.map(i => (
                  <div key={i.service.id} className="flex items-center gap-2 text-sm">
                    ...
                    <button onClick={() => changeQty(i.service.id, -i.quantity)} className="text-gray-400 hover:text-red-500 cursor-pointer"><X className="h-3.5 w-3.5" /></button>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="mt-3 p-2.5 bg-gray-50 rounded-md text-xs space-y-2">
                ...
                {selectedClient.loyaltyPoints >= 10 && cart.length > 0 && (
                  ...
                  >
                    ...
                    <div className={cn('h-4 w-4 rounded border-2 flex items-center justify-center', redeemPoints ? 'bg-green-600 border-green-600' : 'border-gray-300')}>
          ...
          <div className="px-4 pb-4 flex gap-2">
            {step > 1 && <button onClick={() => setStep(s => s - 1)} className="btn-secondary flex-none"><ChevronLeft className="h-4 w-4" /></button>}
            {step < 3
              ? <button onClick={() => setStep(s => s + 1)} disabled={step === 1 ? cart.length === 0 : clientId === null || staffId === null} className="btn-primary flex-1 disabled:opacity-40 disabled:cursor-not-allowed">Continue <ArrowRight className="h-4 w-4" /></button>
